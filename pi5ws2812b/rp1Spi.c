...
/// @brief Writes 32 bits of data to the SPI bus, blocking until it can write and until the write is complete
...
spi_status_t rp1_spi_write_32_blocking(rp1_spi_instance_t *spi, uint32_t data)
{
    ...
    *(volatile uint32_t *)(spi->regbase + DW_SPI_DR) = data;
    ...
    uint8_t discard = *(volatile uint32_t *)(spi->regbase + DW_SPI_DR);
