...
int main(void)
{
    ...
    *(volatile uint32_t *)(spi->regbase + DW_SPI_BAUDR) = 40; //Set the frequency to 5Mhz
    ...
    *(volatile uint32_t *)(spi->regbase + DW_SPI_SSIENR) = 0x1;
    ...
    SPI: 
    G: 110110110110110110110110
    R: 100100100100100100100100
    B: 100100100100100100100100
    */
    int foo;
    uint32_t GREEN = 0b110110110110110110110110;
    uint32_t RED = 0b100100100100100100100100;
    ...
    uint32_t picotime;
    res = rp1_spi_write_8_blocking(spi, 0x02);
    if(res != SPI_OK) {
